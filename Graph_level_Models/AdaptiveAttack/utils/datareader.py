...
class GraphData(torch.utils.data.Dataset):
    ...
    def set_fold(self, data):
        ...
        self.labels = [data['labels'][i] for i in range(len(data))]
        self.adj_list = [data['adj_list'][i] for i in range(len(data))]
        self.features = [data['features'][i] for i in range(len(data))]
...
def TuDatasetstoGraph(Datasets,args):
    ...
    data['num_classes'] = int(max(labels) + 1)
    data['n_node_max'] = max(adj_shapes)
    data['num_features'] = len(features[0][0])
    data['labels'] = labels
    data['adj_list'] = adj_list
    data['features'] = features


    args.surrogate_num_features = len(features[0][0])
    args.surrogate_nodemax = max(adj_shapes)
    args.surrogate_num_classes = int(max(labels) + 1)
    return data