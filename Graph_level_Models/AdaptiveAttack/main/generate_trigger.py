...
import Graph_level_Models.AdaptiveAttack.main.benign as benign
...
def run(traindataset,testdataset,final_idx,train_trigger_list,test_trigger_list, surrogate_model,args):
    # Initialize the surrogate model
    model = copy.deepcopy(surrogate_model).to(args.device)

    # pick up initial candidates
    bkd_gids_test, bkd_nids_test, bkd_nid_groups_test = final_idx, train_trigger_list,train_trigger_list
    bkd_gids_test, bkd_nids_test, bkd_nid_groups_test = final_idx, test_trigger_list, test_trigger_list

    nodemax = args.surrogate_nodemax
    featdim = args.surrogate_num_features

    # init two generators for topo/feat
    toponet = gta.GraphTrojanNet(nodemax, args.surrogate_gtn_layernum)
    ...
    init_dr_test = init_trigger(
        args, copy.deepcopy(self.benign_dr), bkd_gids_test, bkd_nid_groups_test, 0.0, 0.0)
