...
class Client_GC():
    def __init__(self, model, train_Loader,test_clean_loader, test_attack_loader, optimizer, device):
        self.model = model.to(device)

        self.train_Loader = train_Loader
        self.test_clean_loader = test_clean_loader
        self.test_attack_loader = test_attack_loader
    ...
    def local_train(self, local_epoch):
        ...
        train_stats = train_gc(self.model, self.train_Loader, self.test_clean_loader, self.optimizer, local_epoch, self.device,self.scaler)
    ...
    def compute_weight_update(self, local_epoch):
        ...
        train_stats = train_gc(self.model, self.dataLoader, self.optimizer, local_epoch, self.device)
    ...
    def evaluate(self):
        return eval_gc(self.model, self.dataLoader['test'], self.device)

    def local_train_prox(self, local_epoch, mu):
        """ For FedProx """
        train_stats = train_gc_prox(self.model, self.dataLoader, self.optimizer, local_epoch, self.device,
    ...
    def evaluate_prox(self, mu):
        return eval_gc_prox(self.model, self.dataLoader['test'], self.device, self.gconvNames, mu, self.W_old)
...
@autocast()
def train_gc(model, train_loader,test_clean_loader, optimizer, local_epoch, device, scaler):
    ...
    for epoch in range(local_epoch):
        ...
        loss_tt, acc_tt = eval_gc(model, test_clean_loader, device)
    ...
    #train_l_sum / batch_count, train_acc_sum / n, test_l, test_acc
    return total_loss, acc, loss_tt,acc_tt
